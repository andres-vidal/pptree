...
// [[Rcpp::export]]
FeatureVector ppforest2_vi_weighted_forest(Forest const& forest,
...
Rcpp::List ppforest2_tree_node_data(Tree const& tree, FeatureMatrix const& x, ResponseVector y) {
  ...
  for (std::size_t i = 0; i < visitor.nodes.size(); ++i) {
    ...
    if (nd.is_leaf) {
      result[i] = Rcpp::List::create(Rcpp::Named("is_leaf") = true,
                                     Rcpp::Named("depth")   = nd.depth,
                                     Rcpp::Named("value")   = nd.value + 1,
                                     Rcpp::Named("groups")  = groups_r);
    } else {
      result[i] = Rcpp::List::create(Rcpp::Named("is_leaf")   = false,
                                     Rcpp::Named("depth")     = nd.depth,
                                     Rcpp::Named("projector") = Rcpp::wrap(nd.projector),
                                     Rcpp::Named("threshold") = nd.threshold,
                                     Rcpp::Named("projected") =
                                         Rcpp::NumericVector(nd.projected_values.begin(), nd.projected_values.end()),
                                     Rcpp::Named("groups") = groups_r);
    }
  }
...
// [[Rcpp::export]]
Rcpp::DataFrame ppforest2_boundary_segments(Tree const& tree,
  ...
  return Rcpp::DataFrame::create(Rcpp::Named("x_start") = xs,
...
// [[Rcpp::export]]
Rcpp::List ppforest2_decision_regions(Tree const& tree,
                                      Rcpp::IntegerVector var_indices,
                                      Rcpp::NumericVector fixed_values,
                                      double x_min,
                                      double x_max,
  ...
  for (std::size_t i = 0; i < visitor.regions.size(); ++i) {
    ...
    result[i] = Rcpp::List::create(Rcpp::Named("x")     = rx,
                                   Rcpp::Named("y")     = ry,
...
Rcpp::List ppforest2_tree_layout(Tree const& tree) {
  ...
  Rcpp::DataFrame edge_df = Rcpp::DataFrame::create(Rcpp::Named("from_x")     = efx,
...
// [[Rcpp::export]]
std::string ppforest2_save_model_json(Model::Ptr model,
