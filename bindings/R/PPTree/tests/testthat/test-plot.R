...
describe("plot.PPTree", {
  ...
  it("tree plot uses C++ node data with histograms", {
    model <- PPTree(Type ~ ., data = iris, seed = 42L)
    nodes <- PPTree:::pptree_tree_node_data(model, model$x, model$y)
  ...
  it("boundary plot uses C++ segment computation", {
    ...
    segs <- PPTree:::pptree_boundary_segments(
  ...
  it("decision regions returns polygon data", {
    ...
    regions <- PPTree:::pptree_decision_regions(
      model, c(0L, 1L), numeric(0),
      4.0, 8.0, 2.0, 4.5
    )
...
describe("plot.PPForest", {
  it("returns a ggplot object for type = 'importance'", {
    model <- PPForest(Type ~ ., data = iris, size = 5, seed = 42L, n_threads = 1)
    p <- plot(model, type = "importance")
    expect_s3_class(p, "ggplot")
  })
