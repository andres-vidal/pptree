...
#define CLI_GOLDEN_TREE_TEST(TestName, dataset, slug, csv, lambda, seed)                                        \
  TEST(CLIGolden, TestName) {                                                                                   \
    ...
    TempFile model;                                                                                             \
    model.clear();                                                                                              \
    auto train = run_ppforest2("-q train -d " + csv +                                                           \
                               " -n 0"                                                                          \
                               " -l " +                                                                         \
...
#define CLI_GOLDEN_FOREST_TEST(TestName, dataset, slug, csv, n_trees, lambda, n_vars, seed)                       \
  TEST(CLIGolden, TestName) {                                                                                     \
    ...
    model.clear();                                                                                                \
    auto train =                                                                                                  \
        run_ppforest2("-q train -d " + csv + " -n " + std::to_string(n_trees) + " -l " + std::to_string(lambda) + \
                      " -r " + std::to_string(seed) + " -v " + std::to_string(n_vars) +                           \
                      " --threads 1"                                                                              \
                      " -s " +                                                                                    \
                      model.path());                                                                              \
    ASSERT_EQ(train.exit_code, 0) << "train failed";                                                              \
