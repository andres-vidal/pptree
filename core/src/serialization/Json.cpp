...
namespace ppforest2::serialization {
  ...
  template<> json Export<Model::Ptr>::to_json() const {
    ...
    result["training_confusion_matrix"] =
        training_confusion_matrix ? serialization::to_json(*training_confusion_matrix, groups) : json(nullptr);

    result["oob_error"] = oob_error;

    result["oob_confusion_matrix"] =
        oob_confusion_matrix ? serialization::to_json(*oob_confusion_matrix, groups) : json(nullptr);

    result["training_regression_metrics"] =
        training_regression_metrics ? serialization::to_json(*training_regression_metrics) : json(nullptr);

    result["oob_regression_metrics"] =
        oob_regression_metrics ? serialization::to_json(*oob_regression_metrics) : json(nullptr);

    result["variable_importance"] = variable_importance ? serialization::to_json(*variable_importance) : json(nullptr);
  ...
  template<> Tree::Ptr from_json<Tree::Ptr>(json const& j) {
    return std::make_unique<ClassificationTree>(node_from_json(j["root"]), placeholder_classification_spec());
  }
  ...
  template<> Forest::Ptr from_json<Forest::Ptr>(json const& j) {
    ...
    for (auto const& tree_json : j.at("trees")) {
      ...
      std::unique_ptr<Tree> inner_tree =
          std::make_unique<ClassificationTree>(node_from_json(tree_json.at("root")), placeholder_classification_spec());

      forest->add_tree(std::make_unique<BaggedTree>(std::move(inner_tree), std::move(sample_indices)));
    }
  ...
  template<> void Export<Model::Ptr>::compute_metrics(types::FeatureMatrix const& x, types::OutcomeVector const& y) {
    ...
    struct MetricsVisitor : Model::Visitor {
      ...
      MetricsVisitor(
          FeatureMatrix const& x, OutcomeVector const& y, int seed, bool is_regression, Export<Model::Ptr>& self
      )
          : x(x)
          , y(y)
          , seed(seed)
      ...
      void visit(Forest const& forest) override {
        ...
        for (int i = 0; i < oob_preds.size(); ++i) {
          bool const no_oob =
              is_regression ? std::isnan(static_cast<double>(oob_preds(i))) : oob_preds(i) == Outcome(-1);
      ...
      void visit(Tree const& tree) override { self.variable_importance = tree.variable_importance(x); }
    };
...
namespace nlohmann {
  ...
  Export<Tree::Ptr> adl_serializer<Export<Tree::Ptr>>::from_json(json const& j) {
    ...
    auto root = is_regression ? node_from_json(j.at("model")["root"]) : node_from_json(j.at("model")["root"], groups);
  ...
  Export<Forest::Ptr> adl_serializer<Export<Forest::Ptr>>::from_json(json const& j) {
    ...
    Forest::Ptr forest = is_regression ? static_cast<Forest::Ptr>(std::make_unique<RegressionForest>(spec))
    ...
    for (auto const& tree_json : mj.at("trees")) {
      ...
      auto root = is_regression ? node_from_json(tree_json.at("root")) : node_from_json(tree_json.at("root"), groups);
      ...
      forest->add_tree(std::make_unique<BaggedTree>(std::move(inner), std::move(sample_indices)));
    }
  ...
  Export<Model::Ptr> adl_serializer<Export<Model::Ptr>>::from_json(json const& j) {
    ...
    if (has_value(j, "training_regression_metrics")) {
      result.training_regression_metrics =
          serialization::from_json<RegressionMetrics>(j["training_regression_metrics"]);
    }
