...
namespace ppforest2::serialization {
  namespace {
    ...
    std::string validate_config(JsonReader const& config) {
      for (char const* key : CONFIG_REQUIRED_BASE) {
        if (!config.contains(key)) {
          throw std::runtime_error(config.path() + "." + key + ": missing required key");
        }
      }
    ...
    void validate_meta(JsonReader const& meta, std::string const& config_mode) {
      ...
      if (meta.contains("mode")) {
        ...
        if (meta_mode != config_mode) {
          throw std::runtime_error(
              meta.path() + ".mode: disagrees with config.mode (meta='" + meta_mode + "', config='" + config_mode + "')"
          );
        }
      }
      ...
      if (meta.contains("feature_names")) {
        ...
        for (std::size_t i = 0; i < names.size(); ++i) {
          if (!names[i].is_string()) {
            throw std::runtime_error(
                meta.path() + ".feature_names[" + std::to_string(i) + "]: expected string, got " + names[i].type_name()
            );
          }
        }
      }
      ...
      if (is_regression && has_groups) {
        throw std::runtime_error(meta.path() + ".groups: must be absent for regression models (mode='regression')");
      }
      if (!is_regression) {
        if (!has_groups) {
          throw std::runtime_error(
        ...
        if (groups.empty()) {
          throw std::runtime_error(meta.path() + ".groups: must be non-empty for classification models");
        }
        for (std::size_t i = 0; i < groups.size(); ++i) {
          if (!groups[i].is_string()) {
            throw std::runtime_error(
                meta.path() + ".groups[" + std::to_string(i) + "]: expected string, got " + groups[i].type_name()
            );
          }
        }
      }
  ...
  void validate_tree_export(nlohmann::json const& j) {
    ...
    if (root.contains("model_type")) {
      ...
      if (t != "tree") {
        throw std::runtime_error("Export.model_type: expected 'tree' (got '" + t + "')");
      }
    }
  ...
  void validate_forest_export(nlohmann::json const& j) {
    ...
    if (root.contains("model_type")) {
      std::string const t = root.require_enum("model_type", {"tree", "forest"});
      if (t != "forest") {
        throw std::runtime_error("Export.model_type: expected 'forest' (got '" + t + "')");
      }
    }
