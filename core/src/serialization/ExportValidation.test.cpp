...
namespace {
  testing::AssertionResult throws_with(
    ...
    try {
      ...
      if (what.find(needle) != std::string::npos) return testing::AssertionSuccess();
...
TEST(ExportValidation, RejectsModeMismatch) {
  ...
  EXPECT_TRUE(throws_with(
      [&] { validate_model_export(j); },
      "Export.meta.mode: disagrees with config.mode"
  ));
}
...
TEST(ExportValidation, RejectsRegressionWithGroups) {
  ...
  EXPECT_TRUE(throws_with(
      [&] { validate_model_export(j); },
      "Export.meta.groups: must be absent for regression"
  ));
}
...
TEST(ExportValidation, AllGoldenFilesValidate) {
  ...
  for (auto const& entry : std::filesystem::recursive_directory_iterator(golden_dir)) {
    ...
    EXPECT_NO_THROW(validate_model_export(j))
        << "golden file failed validation: " << entry.path();
    ++checked;
  }
