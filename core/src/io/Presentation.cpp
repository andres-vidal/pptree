...
namespace ppforest2::io {
  nlohmann::json ModelStats::to_json() const {
    nlohmann::json j = {{"runs", tr_times.size()},
  ...
  void print_variable_importance(Output& out,
    ...
    if (!all_ones) {
      out.newline();
      out.newline();
      out.println("{} Variable importance was calculated using scaled coefficients (|a_j| * \u03c3_j).",
  ...
  void print_confusion_matrix(Output& out,
  ...
  void print_configuration(Output& out, nlohmann::json const& config, ConfigDisplayHints const& hints) {
    ...
    if (size > 0) {
      ...
      out.println(
          "{}",
          format_row(columns,
                     {"threads", fmt::format("{}{}", config.value("threads", 1), dtag(hints.default_threads))}));
    }
