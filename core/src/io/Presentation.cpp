...
namespace ppforest2::io {
  void print_results(Output& out, ModelStats const& stats) {
    ...
    std::string const tr_err =
        as_rate ? fmt::format("{:.2f}%", stats.mean_tr_error() * 100) : fmt::format("{:.4f}", stats.mean_tr_error());
    std::string const te_err =
        as_rate ? fmt::format("{:.2f}%", stats.mean_te_error() * 100) : fmt::format("{:.4f}", stats.mean_te_error());
