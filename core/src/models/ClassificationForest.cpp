...
namespace ppforest2 {
  namespace {
     ...
     */
    std::vector<int>
    stratified_sample(FeatureMatrix const& /*x*/, GroupPartition const& group_spec, RNG& rng) {
    ...
    BaggedTree::Ptr train_classification_bag(
      ...
      // Reusing the parent's `group_spec` over the resampled `sampled_x`
      ...
      if (!group_spec.groups.empty()) {
        GroupId const first = *group_spec.groups.begin();
        invariant(
            group_spec.group_start(first) == 0,
            "classification bag: group_spec must start at row 0"
        );
      }

      std::vector<int> sample_indices = stratified_sample(x, group_spec, rng);
      FeatureMatrix sampled_x         = x(sample_indices, Eigen::all);

      ClassificationTree::Ptr tree =
          ClassificationTree::train(*training_spec, sampled_x, group_spec, rng);
  ...
  ClassificationForest::Ptr ClassificationForest::train(
      TrainingSpec const& training_spec, FeatureMatrix const& x, OutcomeVector const& y
  ) {
    invariant(
        training_spec.mode == Mode::Classification,
        "ClassificationForest::train requires mode = Classification"
    );
    ...
    GroupPartition group_spec = training_spec.init_groups(y);
    ...
    for (int i = 0; i < size; ++i) {
      for (int attempt = 0; attempt <= max_retries; ++attempt) {
        try {
          ...
          uint64_t stream =
              static_cast<uint64_t>(i) + static_cast<uint64_t>(attempt) * static_cast<uint64_t>(size);
          RNG rng(static_cast<uint64_t>(seed), stream);
          boots[i] = train_classification_bag(spec, x, group_spec, rng);
  ...
  std::optional<double>
  ClassificationForest::oob_error(FeatureMatrix const& x, OutcomeVector const& y) const {
  ...
  // -------------------------------------------------------------------------
  FeatureVector ClassificationForest::vi_permuted(
