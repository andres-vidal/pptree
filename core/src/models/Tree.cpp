...
namespace ppforest2 {
  ...
  TreeCondition::Ptr binary_step(TrainingSpec const& training_spec,
  ...
  GroupPartition binary_split(TrainingSpec const& training_spec,
                              FeatureMatrix const& x,
                              GroupPartition const& group_spec,
                              DRResult const& dr) {
    PPStrategy const& pp_strategy = *training_spec.pp_strategy;
  ...
  build_root(TrainingSpec const& training_spec, FeatureMatrix const& x, GroupPartition const& y, stats::RNG& rng) {
    ...
    while (!stack.empty()) {
      ...
      if (step.pop) {
        *step.node = TreeCondition::make(step.projector,
                                         step.threshold,
                                         std::move(step.lower),
                                         std::move(step.upper),
                                         step.y.groups,
  ...
  Tree Tree::train(TrainingSpec const& training_spec,
                   FeatureMatrix const& x,
                   GroupPartition const& group_spec,
                   stats::RNG& rng) {
