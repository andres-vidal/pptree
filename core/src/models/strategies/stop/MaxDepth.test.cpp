...
TEST(MaxDepthStop, StopsWhenDepthReachesMax) {
  ...
  FeatureMatrix const x = MAT(Feature, rows(2), 1, 2, 3, 4);
  GroupIdVector const y = VEC(GroupId, 0, 1);
  GroupPartition const gp(y);
  RNG rng(0);

  MaxDepth const rule(3);
  EXPECT_TRUE(rule.should_stop(NodeContext(x, gp, 3), rng));
  EXPECT_TRUE(rule.should_stop(NodeContext(x, gp, 4), rng));
  EXPECT_FALSE(rule.should_stop(NodeContext(x, gp, 2), rng));
}
...
TEST(MaxDepthStop, ZeroMaxDepthProducesStump) {
  ...
  EXPECT_TRUE(rule.should_stop(NodeContext(x, gp, 0), rng));
}
