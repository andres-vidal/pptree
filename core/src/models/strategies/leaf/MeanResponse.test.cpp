...
TEST(MeanResponseLeaf, MeanOfAllObservations) {
  ...
  OutcomeVector const ov = VEC(Feature, 1.0, 3.0, 5.0, 7.0);
  RNG rng(0);
...
TEST(MeanResponseLeaf, MeanOfSingleGroup) {
  ...
  GroupPartition const gp(y);
  OutcomeVector const ov = VEC(Feature, 2.0, 4.0, 6.0);
  RNG rng(0);
...
TEST(MeanResponseLeaf, MeanOfSubset) {
  ...
  OutcomeVector const ov = VEC(Feature, 10.0, 20.0, 30.0, 40.0);
  RNG rng(0);

  NodeContext ctx(x, sub, 0);
  ctx.y_vec = &ov;
