...
namespace {
  ...
  BaggedTree::Ptr train_regression_bag(
    ...
    std::stable_sort(order.begin(), order.end(), [&sampled_y](int a, int b) {
      return sampled_y(a) < sampled_y(b);
    });
    ...
    RegressionTree::Ptr tree =
        RegressionTree::train(*training_spec, sorted_x, sampled_gp, rng, sorted_y);
...
namespace ppforest2 {
  ...
  RegressionForest::Ptr RegressionForest::train(
    ...
    invariant(
        ...
        "RegressionForest::train requires mode = Regression"
    );
    ...
    // Note: we do not compute a forest-level `group_spec` here —
  ...
  FeatureMatrix RegressionForest::predict(FeatureMatrix const& /*data*/, Proportions) const {
    throw std::invalid_argument(
        "Vote proportions are not available for regression forests. "
  ...
  OutcomeVector RegressionForest::oob_predict(FeatureMatrix const& x) const {
    ...
    for (int i = 0; i < n_total; ++i) {
      ...
      out(i)  = cnt > 0
          ? static_cast<Outcome>(sums[static_cast<std::size_t>(i)] / static_cast<Feature>(cnt))
          : no_oob;
    }
  ...
  std::optional<double>
  RegressionForest::oob_error(FeatureMatrix const& x, OutcomeVector const& y) const {
    user_error(
        y.size() == x.rows(),
  ...
  // -------------------------------------------------------------------------
  FeatureVector RegressionForest::vi_permuted(
