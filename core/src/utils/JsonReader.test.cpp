...
namespace {
  testing::AssertionResult throws_with(
      std::function<void()> const& fn, std::string const& needle
  ) {
    try {
      fn();
      ...
      return testing::AssertionFailure()
             << "expected message containing \"" << needle
             << "\", got \"" << what << "\"";
    } catch (std::exception const& e) {
      return testing::AssertionFailure()
             << "threw std::exception (not runtime_error): " << e.what();
    }
...
TEST(JsonReader, RequireEnumWrongType) {
  ...
  JsonReader const r(j, "config");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_enum("mode", {"a", "b"}); },
      "config.mode: expected string"
  ));
}
...
TEST(JsonReader, RequireIntOutOfRange) {
  ...
  JsonReader const r(j, "config");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_int("size", 0); }, "config.size: must be in [0, ∞]"
  ));
}
...
TEST(JsonReader, RequireIntRejectsNonIntegerFloat) {
  ...
  JsonReader const r(j, "config");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_int("size"); }, "config.size: expected integer"
  ));
}
...
TEST(JsonReader, RequireIntRejectsFloatsOutsideLongLongRange) {
  ...
  JsonReader const r(j, "config");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_int("k"); },
      "config.k: integer out of representable range"
  ));
}
...
TEST(JsonReader, RequireNumberOutOfRange) {
  ...
  JsonReader const r(j, "pp");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_number("lambda", 0.0, 1.0); },
      "pp.lambda: must be in [0, 1]"
  ));
}
...
TEST(JsonReader, OnlyKeysRejectsUnknown) {
  ...
  EXPECT_TRUE(throws_with(
      ...
      "pp: unexpected key 'lamda' (allowed: name, lambda)"
  ));
}
...
TEST(JsonReader, RequireArrayOnNonArrayThrows) {
  ...
  JsonReader const r(j, "stop");
  EXPECT_TRUE(throws_with(
      [&] { (void)r.require_array("rules"); }, "stop.rules: expected array"
  ));
}
