...
namespace ppforest2::stats {
  ...
  DataPacket
  simulate_regression(int const n, int const p, RNG& rng, RegressionSimulationParams const& params) {
    using namespace types;

    int const n_informative =
        params.n_informative > 0 ? std::min(params.n_informative, p) : std::min(p, 5);
