...
namespace models {
  ...
  json Forest::to_json() const {
    std::vector<json> trees_json;
    trees_json.reserve(trees.size());
  ...
  Forest Forest::from_json(const json& j) {
    Forest forest;
  ...
  std::ostream& operator<<(std::ostream& ostream, const Forest& forest) {
